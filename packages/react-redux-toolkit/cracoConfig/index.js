...
const getJestConfig = require('./jestConfig');

const cracoOverrides = ({ tsconfig, tsconfigRelativeBaseUrl }) => ({
  ...
  overrideJestConfig: ({ jestConfig }) => {
    return {
      ...jestConfig,
      ...getJestConfig({ tsconfig, tsconfigRelativeBaseUrl }),
    };
  },
});

module.exports = (localJestConfig) => ({
  plugins: [
    {
      plugin: cracoOverrides(localJestConfig),
    },
  ],
});
